...
킖lock.scriptletFilteringEngine = (function() {
    const 킶 = 킖lock;
    const duplicates = new Set();
    const scriptletCache = new 킶.MRUCache(32);
    const reEscapeScriptArg = /[\\'"]/g;

    const scriptletDB = new 킶.staticExtFilteringEngine.HostnameBasedDB(1);
    const sessionScriptletDB = new (
        class extends 킶.staticExtFilteringEngine.SessionDB {
            compile(s) {
    ...
    api.retrieve = function(request) {
        ...
        if ( 킶.hiddenSettings.ignoreScriptInjectFilters ) { return; }

        const reng = 킶.redirectEngine;
        if ( !reng ) { return; }
        ...
        if (
            킶.userSettings.advancedUserEnabled &&
            킶.sessionFirewall.evaluateCellZY(hostname, hostname, '*') === 2
        ) {
            return;
        }
        ...
        const loggerEnabled = 킶.logger.enabled;
        ...
        if ( 킶.hiddenSettings.debugScriptlets ) {
    ...
    api.benchmark = async function() {
        ...
        for ( let i = 0; i < requests.length; i++ ) {
            ...
            details.hostname = 킶.URI.hostnameFromURI(request.url);
            details.domain = 킶.URI.domainFromHostname(details.hostname);
            details.entity = 킶.URI.entityFromDomain(details.domain);
