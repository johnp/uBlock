...
킖lock.staticExtFilteringEngine = (( ) => {
    const 킶 = 킖lock;
    ...
    const api = {
        get acceptedCount() {
            return 킶.cosmeticFilteringEngine.acceptedCount +
                   킶.scriptletFilteringEngine.acceptedCount +
                   킶.htmlFilteringEngine.acceptedCount;
        },
        get discardedCount() {
            return 킶.cosmeticFilteringEngine.discardedCount +
                   킶.scriptletFilteringEngine.discardedCount +
                   킶.htmlFilteringEngine.discardedCount;
        },
    };
    ...
    api.reset = function() {
        ...
        킶.cosmeticFilteringEngine.reset();
        킶.scriptletFilteringEngine.reset();
        킶.htmlFilteringEngine.reset();
        resetParsed(parsed);
    };

    api.freeze = function() {
        compileProceduralSelector.reset();
        킶.cosmeticFilteringEngine.freeze();
        킶.scriptletFilteringEngine.freeze();
        킶.htmlFilteringEngine.freeze();
        resetParsed(parsed);
    };
    ...
    api.compile = function(raw, writer) {
        ...
        if ( c0 === 0x2B /* '+' */ && suffix.startsWith('+js') ) {
            킶.scriptletFilteringEngine.compile(parsed, writer);
            return true;
        }
        ...
        //       filtering syntax.
        if ( c0 === 0x5E /* '^' */ ) {
            킶.htmlFilteringEngine.compile(parsed, writer);
            return true;
        }

        // Cosmetic filtering engine.
        킶.cosmeticFilteringEngine.compile(parsed, writer);
        return true;
    };

    api.fromCompiledContent = function(reader, options) {
        킶.cosmeticFilteringEngine.fromCompiledContent(reader, options);
        킶.scriptletFilteringEngine.fromCompiledContent(reader, options);
        킶.htmlFilteringEngine.fromCompiledContent(reader, options);
    };

    api.toSelfie = function(path) {
        return 킖lock.assets.put(
            `${path}/main`,
            JSON.stringify({
                cosmetic: 킶.cosmeticFilteringEngine.toSelfie(),
                scriptlets: 킶.scriptletFilteringEngine.toSelfie(),
                html: 킶.htmlFilteringEngine.toSelfie()
            })
        );
    };

    api.fromSelfie = function(path) {
        return 킖lock.assets.get(`${path}/main`).then(details => {
            let selfie;
            try {
            ...
            킶.cosmeticFilteringEngine.fromSelfie(selfie.cosmetic);
            킶.scriptletFilteringEngine.fromSelfie(selfie.scriptlets);
            킶.htmlFilteringEngine.fromSelfie(selfie.html);
            return true;
        });
    };
