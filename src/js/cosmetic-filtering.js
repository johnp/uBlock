...
const FilterContainer = function() {
    ...
    // specific filters
    this.specificFilters = new 킶.staticExtFilteringEngine.HostnameBasedDB(2);

    // temporary filters
    this.sessionFilterDB = new (
        class extends 킶.staticExtFilteringEngine.SessionDB {
            compile(s) {
                return 킶.staticExtFilteringEngine.compileSelector(s);
            }
        }
    )();
...
FilterContainer.prototype.compileGenericHideSelector = function(
    ...
    const compiled = 킶.staticExtFilteringEngine.compileSelector(selector);
    ...
    if (
        compiled === undefined ||
        compiled !== selector &&
        킶.staticExtFilteringEngine.compileSelector.pseudoclass !== true
    ) {
        if ( 킶.hiddenSettings.allowGenericProceduralFilters === true ) {
...
FilterContainer.prototype.compileGenericUnhideSelector = function(
    ...
    const compiled = 킶.staticExtFilteringEngine.compileSelector(parsed.suffix);
