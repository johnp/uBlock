...
킖lock.staticNetFilteringEngine = (( ) => {

/******************************************************************************/

const 킶 = 킖lock;
const urlTokenizer = 킶.urlTokenizer;
...
const bidiTrie = (( ) => {
    ...
    const trie = new 킶.BidiTrieContainer(trieDetails, bidiTrieMatchExtra);
    if ( 킶.hiddenSettings.disableWebAssembly !== true ) {
        trie.enableWASM();
    }
    return trie;
})();
...
/*******************************************************************************
    ...
    reflected with 킖lock.systemSettings.compiledMagic.
...
const filterOrigin = new (class {
    constructor() {
        ...
        this.trieContainer = new 킶.HNTrieContainer(trieDetails);
...
FilterHostnameDict.trieContainer = (( ) => {
    ...
    return new 킶.HNTrieContainer(trieDetails);
})();
...
FilterContainer.prototype.freeze = function() {
    ...
    const unserialize = 킶.CompiledLineIO.unserialize;
    const units = filterUnits;
    ...
    for ( const line of this.goodFilters ) {
        ...
        if ( (bits & 0x1F0) === redirectTypeValue ) {
            킶.redirectEngine.fromCompiledRule(args[1]);
            continue;
        }
...
FilterContainer.prototype.toSelfie = function(path) {
    ...
    return Promise.all([
        킶.assets.put(
            ...
            FilterHostnameDict.trieContainer.serialize(킶.base64)
        ),
        킶.assets.put(
            `${path}/FilterOrigin.trieContainer`,
            filterOrigin.trieContainer.serialize(킶.base64)
        ),
        킶.assets.put(
            `${path}/bidiTrie`,
            bidiTrie.serialize(킶.base64)
        ),
        킶.assets.put(
...
FilterContainer.prototype.fromSelfie = function(path) {
    return Promise.all([
        킶.assets.get(`${path}/FilterHostnameDict.trieContainer`).then(details =>
            FilterHostnameDict.trieContainer.unserialize(
                details.content,
                킶.base64
            )
        ),
        킶.assets.get(`${path}/FilterOrigin.trieContainer`).then(details =>
            filterOrigin.trieContainer.unserialize(
                details.content,
                킶.base64
            )
        ),
        킶.assets.get(`${path}/bidiTrie`).then(details =>
            bidiTrie.unserialize(
                details.content,
                킶.base64
            )
        ),
        킶.assets.get(`${path}/filterSequences`).then(details => {
            const size = 킶.base64.decodeSize(details.content) >> 2;
            if ( size === 0 ) { return false; }
            filterSequenceBufferResize(size);
            filterSequences = 킶.base64.decode(
                details.content,
                filterSequences.buffer
            );
        ...
        킶.assets.get(`${path}/main`).then(details => {
            let selfie;
            try {
...
FilterContainer.prototype.compileRedirectRule = function(parsed, writer) {
    const redirects = 킶.redirectEngine.compileRuleFromStaticFilter(parsed.raw);
...
FilterContainer.prototype.matchStringElementHide = function(type, url) {
    ...
    $docHostname = $requestHostname = 킶.URI.hostnameFromURI(url);
...
FilterContainer.prototype.benchmark = async function(action, target) {
    const requests = await 킶.loadBenchmarkDataset();
