...
const FrameStore = class {
    ...
    init(frameURL) {
        const 킶uri = 킶.URI;
        this.pageHostname = 킶uri.hostnameFromURI(frameURL);
        this.pageDomain =
            킶uri.domainFromHostname(this.pageHostname) || this.pageHostname;
        return this;
    }
...
const PageStore = class {
    ...
    init(tabId, context) {
        ...
        this.noCosmeticFiltering = 킶.sessionSwitches.evaluateZ(
        ...
        if (
            ...
            fctxt.setRealm('cosmetic')
                 .setType('dom')
                 .setFilter(킶.sessionSwitches.toLogData())
                 .toLogger();
        }
    ...
    reuse(context) {
        ...
        const tabContext = 킶.tabContextManager.mustLookup(this.tabId);
    ...
    toggleNetFilteringSwitch(url, scope, state) {
        킶.toggleNetFilteringSwitch(url, scope, state);
        this.netFilteringCache.empty();
    }
    ...
    injectLargeMediaElementScriptlet() {
        ...
        킶.contextMenu.update(this.tabId);
    }
    ...
    temporarilyAllowLargeMediaElements(state) {
        ...
        킶.scriptlets.injectDeep(this.tabId, 'load-large-media-all');
    }
    ...
    journalAddRequest(hostname, result) {
        ...
        if ( this.journalTimer === null ) {
            this.journalTimer = vAPI.setTimeout(
                ...
                킶.hiddenSettings.requestJournalProcessPeriod
            );
        }
    }
    ...
    journalProcess(fromTimer) {
        ...
        if ( (aggregateCounts & 0xFFFF) && 킶.userSettings.showIconBadge ) {
            킶.updateToolbarIcon(this.tabId, 0x02);
        }
        ...
        if ( aggregateCounts !== 0 ) {
            킶.localSettings.blockedRequestCount +=
                aggregateCounts & 0xFFFF;
            킶.localSettings.allowedRequestCount +=
                aggregateCounts >>> 16 & 0xFFFF;
            킶.localSettingsLastModified = now;
        }
        journal.length = 0;
    }
    ...
    filterRequest(fctxt) {
        ...
        let result = 킶.sessionURLFiltering.evaluateZ(
            fctxt.getTabHostname(),
            fctxt.url,
            requestType
        );
        if ( result !== 0 && 킶.logger.enabled ) {
            fctxt.filter = 킶.sessionURLFiltering.toLogData();
        }

        // Dynamic hostname/type filtering.
        if ( result === 0 && 킶.userSettings.advancedUserEnabled ) {
            result = 킶.sessionFirewall.evaluateCellZY(
                fctxt.getTabHostname(),
                fctxt.getHostname(),
                requestType
            );
            if ( result !== 0 && result !== 3 && 킶.logger.enabled ) {
                fctxt.filter = 킶.sessionFirewall.toLogData();
            }
        }

        // Static filtering has lowest precedence.
        if ( result === 0 || result === 3 ) {
            result = 킶.staticNetFilteringEngine.matchString(fctxt);
            if ( result !== 0 && 킶.logger.enabled ) {
                fctxt.filter = 킶.staticNetFilteringEngine.toLogData();
            }
        }
    ...
    filterCSPReport(fctxt) {
        if (
            킶.sessionSwitches.evaluateZ(
                'no-csp-reports',
                fctxt.getHostname()
            )
        ) {
            if ( 킶.logger.enabled ) {
                fctxt.filter = 킶.sessionSwitches.toLogData();
            }
            return 1;
        }
        return 0;
    ...
    filterLargeMediaElement(fctxt, size) {
        ...
        if ( (size >>> 10) < 킶.userSettings.largeMediaSize ) {
            return 0;
        }
    ...
    getBlockedResources(request, response) {
        const normalURL = 킶.normalizePageURL(this.tabId, request.frameURL);
        const resources = request.resources;
        const fctxt = 킶.filteringContext;
